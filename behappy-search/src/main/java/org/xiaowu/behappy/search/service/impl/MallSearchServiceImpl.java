...
import org.xiaowu.behappy.api.search.vo.AttrResponseVo;
...
public class MallSearchServiceImpl implements MallSearchService {
    ...
    public SearchResult search(SearchParam param) {
        ...
        try {
            //3、执行检索请求
            SearchResponse response = restHighLevelClient.search(searchRequest, defaultRequestOptions);
            //4、分析响应数据，封装成我们需要的格式
    ...
    private SearchResult buildSearchResult(SearchResponse response,SearchParam param) {
        ...
        if (ArrayUtil.isNotEmpty(hits.getHits())) {
            for (SearchHit hit : hits.getHits()) {
                ...
                if (!StrUtil.isEmpty(param.getKeyword())) {
                    //拿到高亮信息显示标题
    ...
    private SearchRequest buildSearchRequest(SearchParam param) {
        ...
        //1.2.5 skuPrice
        if(!StrUtil.isEmpty(param.getSkuPrice())){
        ...
        if(!StrUtil.isEmpty(param.getSort())){
